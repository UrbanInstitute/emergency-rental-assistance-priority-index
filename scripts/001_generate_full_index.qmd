...
-   **Description**: This script compiles all of the more specialized scripts in the repository to generate the final index dataset comprising indicators, subindexes, and the total weighted index. It also returns z-scored and percentile versions of each variable. Note that this was originally an executable script and has been converted to a .qmd for ease of reviewing and updating.
...
census_year = "2023"          # the year for which to pull Census data
...
metadata = tibble::tibble(
  ...
  chas_year = chas_year,
  outcome_year = outcome_year,
  indicator_index_weights = indicator_index_weights,
  outcome_index_weights = outcome_index_weights)


data_catalog_for_review <- data_catalog_index %>% 
  rename(total_erap_index = index_value,
         housing_subindex = housing_subindex_value,
         income_subindex = income_subindex_value,
         h_characteristics_subindex = hh_chars_value,
         median_monthly_housing_cost = median_monthly_housing_coust,
         percent_renter_occupied_units = percent_renter_occ_units,
         percent_renter_occupied_units_multi_unit = percent_renter_occ_units_multi,
         percent_cost_burdened_renter_hhs = percent_cost_burdened_renters,
         percent_eli = percent_eli_renters,
         average_renter_hh_size = avg_renter_hh_size,
         percent_hispanic = percent_latine,
         eli_total = `Extremely Low-income Renters`)

metadata %>%
  write_csv(here("data", "final-data", paste0("metadata_outcome_year_", outcome_year, "_census_year_", census_year, "_chas_year_", chas_year, ".csv")))

data_catalog_index %>%
  write_csv(here("data", "final-data", paste0("data_catalog_index_outcome_year_", outcome_year, "_census_year_", census_year,  "_chas_year_", chas_year, ".csv")))

comms_index %>%
  write_csv(here("data", "final-data", paste0("comms_index_outcome_year_", outcome_year, "_census_year_", census_year,  "_chas_year_", chas_year, ".csv")))
```
