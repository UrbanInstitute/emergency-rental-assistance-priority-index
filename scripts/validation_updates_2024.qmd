...
## when calculated using public-facing repo, 2021 census year
# comms_index_2021_2018_public_repo = comms_index
comms_index_2021_2018_public_repo %>%
  ...
  mutate(
    index_difference = abs(total_index_actual- total_index_calculated),
    state = str_sub(geoid, 1, 2)) %>%
  #filter(index_difference > 5) #%>% ##312 tracts with index score differences greater than five
  #filter(index_difference == 1) #%>% ##29885 tracts with index scores exactly the same, 31792 with differences of 1
  #summarize(mean_difference = mean(index_difference, na.rm = TRUE), tract_count = n()) %>% ## mean difference of .99 overall
  ggplot() +
...
comms_index_2021_2018_public_repo %>% 
  ...
  left_join(comms_2023_actual %>% select(
    geoid, housing_subindex_actual, hh_chars_subindex_actual, income_subindex_actual, percent_black_actual)) %>%
  filter((housing_subindex_calculated != housing_subindex_actual) | (percent_black_calculated != percent_black_actual))
```
